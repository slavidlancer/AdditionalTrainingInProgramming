function varargout=playshow(action,figHandle);
...
%   $Revision: 1.25 $  $Date: 1998/07/23 22:41:35 $
...
switch action
case 'initialize',
   figHandle=LocalInitFigure(filename);
   if nargout==1
...
function figNumber=LocalInitFigure(filename)
...
slideData.axes=axes(   'Units','normalized', ...
...
slideData.resethandle=LocalBuildBtn('pushbutton', btnNumber, labelStr, callbackStr, 'reset');

% =======The autoplay button===============
btnNumber=4;
labelStr='AutoPlay';
callbackStr='playshow #autoplay';
slideData.autohandle=LocalBuildBtn('pushbutton', btnNumber, labelStr, callbackStr, 'autoPlay');
...
if ~isstruct(slides) | ...
   ...
   slides.text={'You have requested a file which does not exist'
      'or which is not a valid PLAYSHOW file.'};
